...
<div class="page-header">
    <h4>Verifikasi</h4>
    <p>Semua percobaan verifikasi.</p>
</div>
...
function renderTable(rows) {
    ...
    tbody.innerHTML = rows.map(item => `
        <tr>
            <td>
                <span class="row-number">${item.no}</span>
            </td>
            <td>
...
function renderPagination(meta) {
    ...
    if (start > 1) {
        pages += pageBtn(1, '1');
        if (start > 2) pages += `<span style="padding:0 4px;
            color:#94a3b8;font-size:13px;">…</span>`;
    }
    for (let i = start; i <= end; i++)
        pages += pageBtn(i, i, i === currentPage);
    if (end < totalPages) {
        if (end < totalPages - 1) pages += `<span style="padding:0 4px;
            color:#94a3b8;font-size:13px;">…</span>`;
    ...
    <div class="pagination-wrap">
        ...
        <div class="pg-controls">
            ...
            <button onclick="fetchData(${totalPages})"
                    ${currentPage === totalPages ? 'disabled' : ''}
                    class="pg-btn" title="Terakhir">
